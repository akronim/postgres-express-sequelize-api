...
class UserService {
  ...
  async findUsersOrdesDetails(idValue) {
    ...
    const data = await this.userRepository.executeRawQuery(query, [ idValue ], 'SELECT');
  ...
  async findById(idValue, idColumn) {
    const user = await this.userRepository.findById(idValue, idColumn);
    return { user };
  }
  ...
  async updateById(idValue, idColumn, entity) {
    ...
    const user = await this.userRepository.updateById(idValue, idColumn, entity);
    return { user };
  }

  async deleteById(idValue, idColumn) {
    const numAffectedRows = await this.userRepository.deleteById(idValue, idColumn);
    return { deleted: numAffectedRows };
  }

  async executeRawQuery(query, values, queryType) {
    return this.userRepository.executeRawQuery(query, values, queryType);
  }
  ...
  async signInUser(options) {
    ...
    const response = await this.userRepository.executeRawQuery(query, [ email ], 'SELECT');
