...
describe('UserController', () => {
    ...
    server
      ...
      .end((err, res) => {
        ...
        res.body.users.forEach((m) => {
          ...
          expect(m.email).to.satisfy(x => x.endsWith('m'));
          expect(new Date(m.created_at)).to.be.greaterThan(new Date('2018-02-15'));
        });
        done();
      });
  });
  ...
  it('GET: find + filtering + sorting', (done) => {
    ...
    server
      ...
      .end((err, res) => {
        ...
        const emailsResponse = res.body.users.map(x => x.email);
        const emailsDesc = res.body.users.map(x => x.email);
  ...
  it('GET: find + filtering + sorting + pagination', (done) => {
    ...
    server
      ...
      .end((err, res) => {
        ...
        expect(res.body.page).to.be.eq(data.dataRequestOptions.pagination.page);
  ...
  it('GET: findUsersOrdesDetails', (done) => {
    server
      .get(`${BASE_URL}/users/7c0ed405-31ae-4642-99c4-1b0649410790/orders-items`)
      .expect(200)
      .end((err, res) => {
        expect(res.status).to.equal(200);
        ...
        res.body.user_orders_items.forEach((x) => {
          const totalPrice = x.items.reduce((acc, v) => acc + (v.quantity * +v.unit_price), 0);
  ...
  it('PATCH: updateById', (done) => {
    ...
    server
      ...
      .end((err, res) => {
        ...
        expect(res.body.user).to.have.property('password', hashPassword(data.password));
        done();
      });
  });
  ...
  it('DELETE: deleteById', (done) => {
    ...
    server
      ...
      .end((err, res) => {
        expect(res.status).to.equal(200);
        expect(res.body.deleted).to.satisfy(x => typeof x === 'number');
